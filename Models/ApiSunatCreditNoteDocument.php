...
class ApiSunatCreditNoteDocument
{
    ...
    public function construir(int $idnotaCredito): array
    {
        ...
        $tipoOrigen = trim(
            (string)$nota['tipo_documento_modificado']
        );
        ...
        foreach ($detalles as $detalle) {
            ...
            $baseLinea = round((float)$detalle['valor_venta'], 2);
            $igvLinea = round((float)$detalle['igv'], 2);
            $totalLinea = round((float)$detalle['total_linea'], 2);
            $precioConIgv = round(
                (float)$detalle['precio_unitario_con_igv'],
                6
            );
            $valorUnitario = round(
                (float)$detalle['valor_unitario_sin_igv'],
                6
            );
                ...
                'cac:PricingReference' => [
                    'cac:AlternativeConditionPrice' => [
                        'cbc:PriceAmount' => [
                            ...
                            '_text' => $this->numeroJson($precioConIgv, 6)
                        ],
                        'cbc:PriceTypeCode' => [
                            '_text' => '01'
                        ]
        ...
        if (abs($total - $totalRegistrado) > 0.02) {
            throw new RuntimeException(
        ...
        $documentBody = [
            ...
            'cac:DiscrepancyResponse' => [
                ...
                'cbc:Description' => [
                    '_text' => trim((string)$nota['sustento'])
                ]
            ],
            ...
            'cac:AccountingCustomerParty' => [
                'cac:Party' => [
                    ...
                    'cac:PartyLegalEntity' => [
                        'cbc:RegistrationName' => [
                            '_text' => (string)$nota['cliente_nombre']
                        ],
                        'cac:RegistrationAddress' => [
                            'cac:AddressLine' => [
                                'cbc:Line' => [
