...
class Sell{
    ...
    public function insertar($idcliente,$idusuario,$tipo_comprobante,$serie_comprobante,$num_comprobante,$impuesto,$total_venta,$tipo_pago,$num_transac,$idingreso,$idarticulo,$cantidad,$precio_compra,$precio_venta,$descuento){
        ...
        while ($num_elementos < count($idarticulo)) {
            ...
            //$sotckDisponible=$stockVenta['stock_venta'];
            //$stPrecioCompra=$stockVenta['precio_compra'];

//paso ciclo 1 cantVenta=8, sotckDisponible=8
            //RESTANTE A VENDER 8 :STOCK DISPONIBLE 8
            $cantVenta=(int)$cantidad[$num_elementos]; 
            //echo $cantVenta[$num_elementos];
            if($cantVenta<$sotckDisponible){
               // echo 'menor '.$cantVenta;
                ...
                do {
                    ...
                    //$sotckDisponible=$stockVenta['stock_venta'];
                   // $stPrecioCompra=$stockVenta['precio_compra'];
                    //echo $sotckDisponible;
                    //paso do ciclo 1 cantVenta=8, sotckDisponible=8
                    //paso do ciclo 2 cantVenta=0(8), sotckDisponible=0(8)
                    if($cantVenta<$sotckDisponible){ 
                    // echo 'menor '.$cantVenta;
                        $sql_update="UPDATE detalle_ingreso SET stock_venta=stock_venta-'$cantVenta' WHERE iddetalle_ingreso=?";
                        $arrUpdate=array($idViejo);
                    ...
                    //paso do ciclo 1 cantVenta=8, sotckDisponible=8
                        ...
                        //echo 'restante: '.$cantVenta;
                        //echo 'stock: '.$sotckDisponible;
                        $sql_update="UPDATE detalle_ingreso SET stock_venta='0',stock_estado='0' WHERE iddetalle_ingreso=?";
                        $arrUpdate=array($idViejo);
                        $this->conexion->setData($sql_update,$arrUpdate) or $sw=false;

                        //DATOS PARA EL KARDEX
                        //if(empty($sotckDisponible)){
                        //if($a <= $cantVenta){
                        $cantidadExistente=(int)$sotckDisponible-(int)$cantVenta;
                        //echo 'aa: '.$a.'</br>';
//echo 'restante: '.$cantVenta.'</br>';
//echo 'cantidadExistente: '.$cantidadExistente.'</br>';

                    //paso do ciclo 1 cantVenta=8, sotckDisponible=0(8)
                    //cantidadExistente=8-8;
                    //cantidadExistente=0
                       // if($cantidadExistente>0){
                        $totalKardex=$cantVenta*$stPrecioCompra;
                        $totalex=$cantidadExistente*$stPrecioCompra;
                        $costus=$stPrecioCompra*$cantidadExistente;
                        //echo 'stock disponible: '. $sotckDisponible;
                        $sql_kardex="INSERT INTO $this->tableNameKardex (iddetalle,idarticulo,fecha,detalle,cantidads,costous,totals,cantidadex,costouex,totalex,tipo,estado) VALUES(?,?,?,?,?,?,?,?,?,?,?,?)";
                    ...
                    //RESTANTE 3 :STOCK A VENDER 1
                       ...
                       // echo 'otroo cantidadExistente'.$cantidadExistente;
                       // $cant_res=$cantidad[$num_elementos]-$cantidadExistente;
                        //echo $cantVenta;
//if($cantidadExistente>0){
        ...
        $totalex=$sotckDisponible*$precio_compra[$elementos];
        $sql_kardex="INSERT INTO $this->tableNameKardex (iddetalle,idarticulo,fecha,detalle,cantidads,costous,totals,cantidadex,costouex,totalex,tipo,estado) VALUES(?,?,?,?,?,?,?,?,?,?,?,?)";
        $arrKardex = array($idventanew ,$idarticulo[$elementos],$fecha_hora,$detalle,$cantidad[$elementos],$precio_compra[$elementos],$totalKardex,$sotckDisponible,$precio_compra[$elementos],$totalex,'Salida','Activo');
        $this->conexion->setData($sql_kardex,$arrKardex)or $sw=false;
