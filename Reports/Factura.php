...
function addRemarque($remarque)
{
    ...
    $y2  = $y1+5;
    $this->SetXY( $r1 , $y1 );
    $this->Cell($length,4, "Nota : " . $remarque);
...
function addTVAs( $impuesto, $total_venta, $simbolo )
{
    ...
    $length = $this->GetStringWidth( number_format((float)$igv,2,'.',''),0,0,'R');
    $this->Cell( $length+6.5, 2,  number_format((float)$igv,2,'.',''),0,0,'R');
    // mostrando el total
    $this->SetXY( $ret, $y1+15 );
    $length = $this->GetStringWidth( $simbolo.' ' );
    $this->Cell( $length, 2, $simbolo.' ',0,0,'R');

    $length = $this->GetStringWidth( number_format((float)$total_venta,2,'.',''),0,0,'R' );
    $this->Cell( $length+5, 2, number_format((float)$total_venta,2,'.',''),0,0,'R');
...
function temporaire($texte)
{
    ...
    $this->SetTextColor(245, 183, 177);
    $this->Rotate(45,55,190);
    $this->Text(75,210,strtoupper($texte));
    $this->Rotate(0);
