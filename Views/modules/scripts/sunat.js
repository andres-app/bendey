...
/*
|--------------------------------------------------------------------------
| DUPLICAR COMPROBANTE COMO NUEVA VENTA
|--------------------------------------------------------------------------
| Solo abre una plantilla editable. No crea la venta ni consume correlativo.
*/
function duplicarComprobante(idventa) {
    ...
    if (!idventa || idventa <= 0) {
        Swal.fire({
            ...
            text: 'No se pudo determinar el comprobante.'
        });
        return;
    }
    ...
    Swal.fire({
        ...
        html:
            ...
            '<p>Se abrirá una <strong>nueva venta editable</strong> con el cliente y los productos del comprobante.</p>' +
            '<p>El documento original no será modificado y el siguiente correlativo se asignará recién al procesar.</p>' +
            '</div>',
        showCancelButton: true,
        confirmButtonText: 'Abrir nueva venta',
        cancelButtonText: 'Cancelar',
        reverseButtons: true,
        allowOutsideClick: false
    }).then(function (resultado) {
